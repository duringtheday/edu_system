...
export default function RotatingWheel() {
  ...
  const [angle, setAngle] = React.useState(0);        // rotation angle (deg)
  const [active, setActive] = React.useState(0);      // active index

  function rotate(delta: number) {
    const next = (active + delta + MAIN.length) % MAIN.length;
    setActive(next);
    setAngle(-(next * STEP)); // keep selected at top (0deg)
  }
  function go() {
          ...
          return (
            <button
              ...
              style={{
                ...
                transform: `rotate(${-angle}deg)`, // keep icon upright
      ...
      <div className="card animate-slide" style={{
        ...
        <div style={{ textAlign:"center" }}>
          ...
          <div style={{ display:"flex", gap:8, marginTop: 10, justifyContent:"center" }}>
            ...
            <button className="btn btn-primary" onClick={go}>Open</button>
